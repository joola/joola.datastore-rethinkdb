...
RethinkDBProvider.prototype.insert = function (collection, documents, options, callback) {
  ...
  var self = this;
  r.db(self.options.db).tableCreate(collection.key).run(self.client, function (err, result) {
    //ignore errors if table already exists
    r.table(collection.key).insert(documents).run(self.client, function (err, result) {
      if (err)
        return callback(err);
...
RethinkDBProvider.prototype.buildQueryPlan = function (query, callback) {
  ...
  query.metrics.forEach(function (metric) {
    var colQuery = {
      table: metric.collection ? metric.collection.storeKey : null,
      query: []
    };
      ...
      if (metric.key !== 'fake') {
        ...
        switch (metric.aggregation) {
          case 'sum':
            ...
            _$map[metric.aggregation + '_' + metric.key] = metric.aggregation + '_' + metric.key + ': doc("' + metric.key + '")';
            break;
          case 'avg':
...
RethinkDBProvider.prototype.query = function (context, query, callback) {
  ...
  return self.buildQueryPlan(query, function (err, plan) {
    ...
    Object.keys(plan.colQueries).forEach(function (key) {
      ...
      sql += ' from ' + plan.colQueries[key].table.replace(/-/ig, '_');
