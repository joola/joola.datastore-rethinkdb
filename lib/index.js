...
RethinkDBProvider.prototype.buildQueryPlan = function (query, callback) {
  ...
  query.metrics.forEach(function (metric) {
    var colQuery = {
      table: metric.collection.storeKey.replace(/[^\w\s]/gi, ''),
      query: [],
      pipeline: []
    };
        ...
        switch (metric.aggregation) {
          case 'avg':
            if (Object.keys($group).length > 0) {
              ...
              _$reduce['sum_' + metric.dependsOn] = 'sum_' + metric.dependsOn + ': left("sum_' + metric.dependsOn + '").add(right("sum_' + metric.dependsOn + '"))';
              _$reduce['count_' + metric.dependsOn] = 'count_' + metric.dependsOn + ': left("count_' + metric.dependsOn + '").add(right("count_' + metric.dependsOn + '"))';
              _$final[metric.key] = metric.key + ': doc("sum_' + metric.attribute + '").div(doc("count_' + metric.attribute + '"))';
            }
            break;
          case 'distinct':
          case 'unique':
          case 'ucount':
            ...
            _$group[metric.dependsOn] = _$group[metric.dependsOn] || 2;
            _$reduce['count_' + metric.dependsOn] = 'count_' + metric.dependsOn + ': left("' + 'count_' + metric.dependsOn + '").add(right("' + 'count_' + metric.dependsOn + '"))';
            _$final[metric.key] = metric.key + ': doc("reduction")("count_' + metric.dependsOn + '")';
            break;
          case 'max':
            if (Object.keys($group).length > 0) {
              _$map[metric.aggregation + '_' + metric.attribute] = metric.aggregation + '_' + metric.attribute + ': doc("' + metric.attribute + '")';
              _$reduce[metric.aggregation + '_' + metric.attribute] = metric.aggregation + '_' + metric.attribute + ': r.branch(left("' + metric.aggregation + '_' + metric.attribute + '").gt(right("' + metric.aggregation + '_' + metric.attribute + '")), left("' + metric.aggregation + '_' + metric.attribute + '"), right("' + metric.aggregation + '_' + metric.attribute + '"))';
            }
            else {
          ...
          case 'min':
            if (Object.keys($group).length > 0) {
              _$map[metric.aggregation + '_' + metric.attribute] = metric.aggregation + '_' + metric.attribute + ': doc("' + metric.attribute + '")';
              _$reduce[metric.aggregation + '_' + metric.attribute] = metric.aggregation + '_' + metric.attribute + ': r.branch(left("' + metric.aggregation + '_' + metric.attribute + '").lt(right("' + metric.aggregation + '_' + metric.attribute + '")), left("' + metric.aggregation + '_' + metric.attribute + '"), right("' + metric.aggregation + '_' + metric.attribute + '"))';
            }
            else {
              _$map[metric.key] = metric.key + ': doc("' + metric.attribute + '")';
          ...
          default:
            if (Object.keys($group).length > 0) {
              _$map[metric.aggregation + '_' + metric.attribute] = metric.aggregation + '_' + metric.attribute + ': doc("' + metric.attribute + '")';
              _$reduce[metric.aggregation + '_' + metric.attribute] = metric.aggregation + '_' + metric.attribute + ': left("' + metric.aggregation + '_' + metric.attribute + '").add(right("' + metric.aggregation + '_' + metric.attribute + '"))';
            }
            else {
              _$map[metric.key] = metric.key + ': doc("' + metric.attribute + '")';
              _$reduce[metric.key] = metric.key + ': left("' + metric.key + '").add(right("' + metric.key + '"))';
            }
            _$final[metric.key] = metric.key + ': doc("reduction")("' + metric.aggregation + '_' + metric.attribute + '")';
            break;
        }
    ...
    Object.keys(plan.colQueries).forEach(function (key) {
      ...
      var call = function (callback) {
        ...
        if (_plan.type !== 'ucount') {
          ...
          if (Object.keys(queryPlan.$group).length > 0)
            statement = "r.db('" + self.options.db + "').table('" + _plan.table + "')" + filter + ".map(" + map + ").group(" + group + ").reduce(" + reduce + ").ungroup().map(" + final + ").run(self.client, " + cb.toString() + ")";
          else if (Object.keys(queryPlan.$group).length === 0 && query.metrics[0].aggregation !== 'avg')
            statement = "r.db('" + self.options.db + "').table('" + _plan.table + "')" + filter + ".map(" + map + ").reduce(" + reduce + ").run(self.client, " + cb.toString() + ")";
          else if (Object.keys(queryPlan.$group).length === 0 && query.metrics[0].aggregation === 'avg')
            statement = "r.db('" + self.options.db + "').table('" + _plan.table + "')" + filter + ".map(" + map + ").reduce(" + reduce + ").do(" + final + ").run(self.client, " + cb.toString() + ")";
        }
        else {
          ...
          if (Object.keys(queryPlan.$group).length > 0)
            statement = "r.db('" + self.options.db + "').table('" + _plan.table + "')" + filter + ".map(" + map + ").group(" + group + ").ungroup().map(" + map2 + ").group(" + group2 + ").reduce(" + reduce + ").ungroup().map(" + final + ").run(self.client, " + cb.toString() + ")";
          else if (Object.keys(queryPlan.$group).length === 0)
            statement = "r.db('" + self.options.db + "').table('" + _plan.table + "')" + filter + ".map(" + map + ").group(" + group + ").ungroup().map(" + map2 + ").reduce(" + reduce + ").run(self.client, " + cb.toString() + ")";
          else if (Object.keys(queryPlan.$group).length === 0 && query.metrics[0].aggregation === 'avg')
            statement = "r.db('" + self.options.db + "').table('" + _plan.table + "')" + filter + ".map(" + map + ").group(" + group + ").ungroup().map(" + map2 + ").reduce(" + reduce + ").do(" + final + ").run(self.client, " + cb.toString() + ")";
        }
