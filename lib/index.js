        ...
        r.db(self.options.db).table(collection.storeKey).indexCreate('timestamp').run(self.client, function (err, result) {
          ...
          r.table(collection.storeKey).insert(documents).run(self.client, function (err, result) {
            if (err)
              return callback(err);
            if (result.errors > 0)
              return callback(new Error(result.first_error));
...
RethinkDBProvider.prototype.buildQueryPlan = function (query, callback) {
  ...
  if (query.timeframe && !query.timeframe.hasOwnProperty('last_n_items')) {
    ...
    $match.timestamp = {
      $gt: 'r.ISO8601("' + query.timeframe.start.toISOString() + '")',
      $lt: 'r.ISO8601("' + query.timeframe.end.toISOString() + '")'
    };
  }
  ...
  switch (query.interval) {
    case 'timebucket.second':
      query.interval = 'r.time(doc("timestamp").year(), doc("timestamp").month(), doc("timestamp").day(), doc("timestamp").hours(), doc("timestamp").minutes(), doc("timestamp").seconds().coerceTo("string").split(".").nth(0).coerceTo("number"), "Z")';
      break;
    ...
    Object.keys(plan.colQueries).forEach(function (key) {
      ...
      var call = function (callback) {
        var cb = function (err, cursor) {
          if (err) {
            if (err.toString().indexOf('does not exist') > -1)
              return callback(null, []);
            else if (err.toString().indexOf('Cannot reduce over an empty stream') > -1)
              return callback(null, []);
            else
              return callback(err);
          }
          if (typeof cursor === 'object') {
            ...
            //console.log(require('util').inspect(cursor, {depth: null, colors: true}));
        ...
        if (_plan.type !== 'ucount') {
          ...
          if (sort.length > 0 && Object.keys(queryPlan.$group).length > 0)
...
RethinkDBProvider.prototype.checkConnection = function (connection, callback) {
  ...
  r.db(self.options.db).tableList().run(self.client, function (err) {
    ...
    self.logger.info('Connection checked out ok.');
